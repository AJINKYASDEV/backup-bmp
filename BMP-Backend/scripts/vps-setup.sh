...
# Only writes if .env does not already exist — never overwrites a configured file.
JWT_SECRET="$(openssl rand -hex 64)"
if [[ ! -f "$BACKEND_DIR/.env" ]]; then
    log "Generating $BACKEND_DIR/.env ..."
    cat > "$BACKEND_DIR/.env" <<ENVEOF
# Auto-generated by vps-setup.sh — fill in the blanks marked TODO
NODE_ENV=production
PORT=3000
BASE_URL=http://$(curl -s ifconfig.me):3000
FRONTEND_URL=http://$(curl -s ifconfig.me)
ALLOWED_ORIGINS=http://$(curl -s ifconfig.me),http://$(curl -s ifconfig.me):3000

JWT_SECRET=${JWT_SECRET}

DEFAULT_ADMIN_EMAIL=admin@bookmyparcel.co.in
...
# TODO: Add your Google Maps API keys
GOOGLE_API_KEY=
...
# OTP_LENGTH must match booking table VARCHAR(4) column — do NOT change to 6
OTP_LENGTH=4
OTP_MAX_ATTEMPTS=3
OTP_EXPIRY_MINUTES=10
OTP_LOCKOUT_MINUTES=15

# TODO: Add Firebase service account JSON (single line)
...
SMTP_USER=
SMTP_PASS=
SMTP_FROM_NAME=Book My Parcel

TEST_PHONE_NUMBER=
ENVEOF
    chmod 600 "$BACKEND_DIR/.env"
    chown "$APP_USER:$APP_USER" "$BACKEND_DIR/.env"
    log ".env generated — edit it to fill in API keys before starting the app"
else
    warn ".env already exists — skipping generation (not overwriting)"
    # Ensure OTP_LENGTH is correct even if file already exists
    if grep -q "OTP_LENGTH=6" "$BACKEND_DIR/.env"; then
        sed -i 's/OTP_LENGTH=6/OTP_LENGTH=4/' "$BACKEND_DIR/.env"
        warn "Fixed OTP_LENGTH from 6 → 4 (must match VARCHAR(4) DB column)"
    fi
fi

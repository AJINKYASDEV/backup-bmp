  ...
  return (
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center mb-6">
        ...
        <div className="border border-green-300 bg-green-50 rounded-lg px-4 py-3 text-left sm:text-right">
          <p className="text-xs text-gray-500">Total Spent</p>
          <p className="text-green-600 font-bold text-base sm:text-lg break-words">₹{Number(totalSpent || 0).toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {payments.map((item, index) => (
          <div key={index} className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between bg-white p-4 rounded-lg shadow-sm border">
            <div className="flex items-center gap-3 sm:gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <p className="font-semibold text-sm truncate">{item.payment_id}</p>
                <p className="text-xs text-gray-500 truncate">Booking: {item.booking_id}</p>

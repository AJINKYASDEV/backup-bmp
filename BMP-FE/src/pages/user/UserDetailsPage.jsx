        ...
        if (response?.data?.success) {
          ...
          const transformedData = {
            ...
            createdOn: data.createdAt
              ...
              : "—",
            totalDistance: data.route_distance_km ? `${data.route_distance_km} km` : "—",
            avgBidAmount: (() => {
              const acceptances = data.acceptances || [];
              if (acceptances.length === 0) return "—";
              const total = acceptances.reduce((sum, a) => sum + (Number(a.acceptance_price) || 0), 0);
              return `₹${Math.round(total / acceptances.length).toLocaleString("en-IN")}`;
            })(),
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {/* Route & Performance section (Defect 18/19) */}
        {parcelData && (
          <div className="bg-white rounded-2xl p-5 shadow-lg mt-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-sm font-semibold text-gray-800">{parcelData.createdOn}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-xs text-gray-500 mb-1">Total Distance</p>
                <p className="text-sm font-semibold text-gray-800">{parcelData.totalDistance}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-xs text-gray-500 mb-1">Avg. Bid Amount</p>
                <p className="text-sm font-semibold text-gray-800">{parcelData.avgBidAmount}</p>
              </div>
            </div>
          </div>
        )}
